...
function debugLeaders() {
  ...
  if (UI.isInShell()) {
    ...
    for (const {LeaderType, LeaderName} of leaders) {
      ...
      if (colors) {
        const row = fmtLc(LeaderName, colors.primaryColor, colors.secondaryColor);
        leaderLc.push(row);
      }
    }
  } else {
    for (const player of Players.getAlive()) {
      ...
      if (colors) {
        const row = fmtLc(player.leaderName, colors.primaryColor, colors.secondaryColor);
        leaderLc.push(row);
      }
    }
  }
...
if (BZ_DEBUG) debugColors();
if (BZ_DEBUG) debugLeaders();
